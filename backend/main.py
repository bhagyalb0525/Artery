...
@app.post("/artworks/{artwork_id}/buy", status_code=201)
def buy_artwork(artwork_id: int, current_user=Depends(get_current_user), db=Depends(get_db)):
    cur = db.cursor(dictionary=True)
    cur.execute("SELECT * FROM artworks WHERE id = %s", (artwork_id,))
    art = cur.fetchone()
    if not art:
    ...
    cur.execute(
        "INSERT INTO purchases (buyer_id, artwork_id) VALUES (%s, %s)",
        (current_user["id"], artwork_id),
    )
    db.commit()
    cur.close()
    return {"message": f"You purchased '{art['title']}' successfully"}
...
def my_purchases(current_user=Depends(get_current_user), db=Depends(get_db)):
    cur = db.cursor(dictionary=True)
    cur.execute("""
        SELECT a.id, a.title, a.image_path, a.price, p.purchased_at
        FROM purchases p
